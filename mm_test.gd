...
func _ready() -> void:
	...
	if enable_compute:
		# BEST PRACTICE: Defer the compute initialization to ensure 
		# the RenderingServer has allocated the MultiMesh buffer in VRAM.
...
func _init_compute() -> void:
	print("Compute shader logic will initialize here safely!")
	# The MultiMesh RID is now safe to pull from the RenderingServer
